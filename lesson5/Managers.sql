...
-- --------------------------------------------------------------------------------------------------------------
select *, (select Adress from EmploeesDetails  where Emploees.ID = EmploeesDetails.id) as adress from Emploees;
...
select  (select Ename from Emploees where Emploees.ID = EmploeesDetails.ID) as Name, 
		(select Phone from Emploees where Emploees.ID = EmploeesDetails.ID) as Phone,
		BirthDate  from EmploeesDetails where CiwilStatus = 'Single';
...
select  name, 
		(select phone from emploees where emploees.Ename = posittions.name) as phone, 
		(Select birthdate from EmploeesDetails where EmploeesDetails.id = (select id from emploees where emploees.Ename = posittions.name)) as BirthDay 
		from posittions where position = 'Manager';