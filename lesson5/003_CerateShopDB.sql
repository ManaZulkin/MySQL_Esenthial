...
select LName, FName, MName from customers where CustomerNo in (
Select CustomerNo from Orders where OrderID in(
Select OrderID from orderdetails where TotalPrice > 1000))
union
select LName, FName, MName from employees where EmployeeID in (
